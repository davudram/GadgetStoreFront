...
function LaptopCategory() {
    ...
    useEffect(() => {
        ...
        const getGadgetCount = async () => {
            try {
                const response = await axios.get('https://localhost:7108/api/Gadgets/SelectedForCategory?idcategory=2', {
                    headers: {
    ...
    function findGadgets() {
        if (minValue >= 0 && maxValue >= 0 && maxValue > minValue) {
            fetch(`https://localhost:7108/api/Gadgets/FilterPriceByIdCategory?minPrice=${minValue}&maxPrice=${maxValue}&idcategory=2`, {
                headers: {
                    'Authorization': 'Bearer ' + getToken(),
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
