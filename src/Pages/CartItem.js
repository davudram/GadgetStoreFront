...
function CartItem(props) {
    ...
    const [gadgetState, setGadgetState] = useState(props.item.quantity);
    ...
    const addCount = () => {
        ...
        const updatedValue = gadgetState + 1;
        document.getElementById(`quantity-value-${props.item.productId}`).value = updatedValue;
        setGadgetState(updatedValue);
        editCount(updatedValue);
    };
    ...
    const handleDelete = () => {
        props.handleDelCart(props.item.productId);
    };

    return (
        <div className='cart-item' key={props.item.productId}>
            <img src={`${props.item.productImage}`} className="cart-item-img" alt="Product" />
            <div className="cart-item-details">
                <h3>{props.item.productName}</h3>
                <button id='minus-quantity' onClick={removeCount}>-</button>
                <input
                    id={`quantity-value-${props.item.productId}`}
